...
double DAlhood(const LdaSuffStats &ss, LdaModel* m, double* gradient) {
  double alpha_sum = double_array_sum(m->alpha,m->num_topics);
  for(int k=0; k<m->num_topics; ++k) {
    gradient[k] = ss.num_docs * (DiGamma(alpha_sum) - DiGamma(m->alpha[k])) + ss.alpha_suffstats[k];
  }
  return 0.0;
}

double D2Alhood(double* alpha, int d, int k, double* matrix_diagal) {
  double alpha_sum = double_array_sum(alpha,k);
  for(int i=0; i<k; ++i) {
    matrix_diagal[k] = d * TriGamma(alpha[i]) - TriGamma(alpha_sum);
  }
  double non_diagal = TriGamma(alpha_sum) * (-1);
  return non_diagal;
}
...
double OptAlpha(const LdaSuffStats &ss, LdaModel* m) {
  ...
  do {
    ...
    for(int j=0; j<m->num_topics; ++j) {
      diag_sum += matrix_diag[j];
      gradient_diag_ritio_sum += gradient[j]/matrix_diag[j];
    }
    double c = diag_sum / ( 1 / non_diag + gradient_diag_ritio_sum);
